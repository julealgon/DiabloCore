 ...
 */
inline std::byte *CelGetFrame(std::byte *data, int frame)
{
 ...
 */
inline std::byte *CelGetFrame(std::byte *data, int frame, int *frameSize)
{
	const std::uint32_t begin = LoadLE32(&data[frame * sizeof(std::uint32_t)]);
 ...
 * Returns the pointer to the start of the frame data (often a header) and sets `frameSize` to the size of the data in bytes.
 */
inline const std::byte *CelGetFrame(const std::byte *data, int frame, int *frameSize)
{
	const std::uint32_t begin = LoadLE32(&data[frame * sizeof(std::uint32_t)]);
	*frameSize = static_cast<int>(LoadLE32(&data[(frame + 1) * sizeof(std::uint32_t)]) - begin);
	return &data[begin];
 ...
 */
inline const std::byte *CelGetFrameClipped(const std::byte *data, int frame, int *frameSize)
{
	const std::byte *frameData = CelGetFrame(data, frame, frameSize);
