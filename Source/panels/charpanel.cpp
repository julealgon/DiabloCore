...
std::pair<int, int> GetDamage()
{
	...
	int mindam = MyPlayer->_pIDamage.minValue + MyPlayer->_pIBonusDam * MyPlayer->_pIDamage.minValue / 100 + damageMod;
	int maxdam = MyPlayer->_pIDamage.maxValue + MyPlayer->_pIBonusDam * MyPlayer->_pIDamage.maxValue / 100 + damageMod;
...
StyledText GetResistInfo(int8_t resist)
{
	...
	return {
		style, (resist >= MAXRESIST ? "MAX" : fmt::format("{:d}%", resist))
	};
}
...
PanelEntry panelEntries[] = {
	...
	{ "", { 161, 14 }, 149, 0,
	    []() { return StyledText { UiFlags::ColorWhite, ClassStrTbl[static_cast<std::size_t>(MyPlayer->_pClass)] }; } },

	{ "Level", { 57, 52 }, 57, 45,
	    []() { return StyledText { UiFlags::ColorWhite, fmt::format("{:d}", MyPlayer->_pLevel) }; } },
	{ "Experience", { 211, 52 }, 99, 91,
	    []() { return StyledText { UiFlags::ColorWhite, fmt::format("{:d}", MyPlayer->_pExperience) }; } },
	{ "Next level", { 211, 80 }, 99, 198,
	    []() {
	        if (MyPlayer->_pLevel == MAXCHARLEVEL - 1) {
		        return StyledText { UiFlags::ColorWhitegold, "None" };
	        } else {
	...
	{ "Dexterity", { 88, 191 }, 45, 76, []() { return StyledText { GetBaseStatColor(CharacterAttribute::Dexterity), fmt::format("{:d}", MyPlayer->_pBaseDex) }; } },
	{ "", { 135, 191 }, 45, 0,
	    []() { return StyledText { GetCurrentStatColor(CharacterAttribute::Dexterity), fmt::format("{:d}", MyPlayer->_pDexterity) }; } },
	{ "Vitality", { 88, 219 }, 45, 76, []() { return StyledText { GetBaseStatColor(CharacterAttribute::Vitality), fmt::format("{:d}", MyPlayer->_pBaseVit) }; } },
	{ "", { 135, 219 }, 45, 0,
	    []() { return StyledText { GetCurrentStatColor(CharacterAttribute::Vitality), fmt::format("{:d}", MyPlayer->_pVitality) }; } },
	{ "Points to distribute", { 88, 248 }, 45, 76,
	    []() {
...
void DrawShadowString(const Surface &out, const PanelEntry &entry)
{
	...
	std::string text_tmp = entry.label.c_str();
	char buffer[32];
	int spacing = 0;
